...
static struct blk_dev *lkvm_bdev = NULL;
...
static const struct disk_image_params blk_params = {
    .filename   = "/blk-dev.img",
	.wwpn       = NULL,
	.tpgt       = NULL,
    .readonly   = false,
    .direct     = false,
...
void lkvm_blk_init(void);
void lkvm_blk_init(void) {
    disk_image__init(&fake_kvm_for_blk);
    virtio_blk__init(&fake_kvm_for_blk);
}
...
void lkvm_blk_mmio_read(u64 offset, u8 *data, u32 len);
void lkvm_blk_mmio_read(u64 offset, u8 *data, u32 len)
{
    virtio_mmio_read(&lkvm_bdev->vdev, offset, data, len);
}

void lkvm_blk_mmio_write(u64 offset, u8 *data, u32 len);
void lkvm_blk_mmio_write(u64 offset, u8 *data, u32 len)
{
    virtio_mmio_write(&lkvm_bdev->vdev, offset, data, len);
}
