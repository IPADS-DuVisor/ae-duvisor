...
if [ $1 == "sync" ]; then
    ...
    rsync -P -avz -e 'ssh -o "ProxyCommand nc -X 5 -x 192.168.10.1:7890 %h %p" -o StrictHostKeyChecking=no -i ~/Downloads/firesim.pem' -r br-base-bin-laputa mnt-firesim.tar.gz firesim-scripts centos@${IP}:~/firesim
fi
# running laputa
echo running laputa benchmark
ssh -f -o "ProxyCommand nc -X 5 -x 192.168.10.1:7890 %h %p" -o StrictHostKeyChecking=no -i ~/Downloads/firesim.pem centos@${IP} "cd firesim/firesim-scripts && nohup ./scripts-laputa/start.sh laputa > ~/laputa.log 2>&1 &"
